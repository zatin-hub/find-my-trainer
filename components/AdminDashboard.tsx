  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      ...
      <section className="mt-8">
        ...
        {/* Self-measured Ola usage (they expose no usage API — console only) */}
        <div className="mt-4">
            ...
            <div className="overflow-x-auto rounded-lg border border-white/10">
              <table className="w-full text-xs">
                <thead className="bg-white/[0.04] text-slate-400">
                  <tr>
                    {["Day", "Ola tiles", "Ola geocode", "OSM geocode", "Cache hits", "Cache rate", "Ola total"].map(
                      (h) => (
                ...
                <tbody>
                  {usage.map(({ day, counts }) => {
                    ...
                    const hits =
                      (counts.geocode_fwd_cache ?? 0) +
                      (counts.geocode_rev_cache ?? 0);
                    const live =
                      olaGeo +
                      (counts.geocode_fwd_photon ?? 0) +
                    ...
                    return (
                      <tr key={day} className="border-t border-white/5 text-slate-300">
                        ...
                        <td className="px-3 py-1.5 font-medium text-pink-200">
                          {olaTotal}
                        </td>
                      </tr>
                    );
          ...
          <p className="mt-1 text-xs text-slate-500">
            Free tier: 5,000,000 calls/mo per API. Tile counts come from a
            client beacon (batched), so treat as a close estimate; the Ola
            console is the billing source of truth.
          </p>
        </div>
      </section>
